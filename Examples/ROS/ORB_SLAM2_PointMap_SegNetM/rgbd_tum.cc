...
{
    ros::init(argc, argv, "DS_TUM");
    ros::start();
    //::google::InitGoogleLogging(argv[0]);
     if(argc != 10)
    {
        cerr << endl << "Usage: DS_TUM path_to_vocabulary path_to_settings path_to_sequence path_to_association path_to_mean path_to_weights path_to_frac path_to_pascal.png path_to_aocx" << endl;
        return 1;
    }
    ...
    ORB_SLAM2::System SLAM(argv[1],argv[2],argv[5],argv[6],argv[7],argv[8],argv[9],ORB_SLAM2::System::RGBD);
    usleep(50);
    ...
    while(ros::ok()&&ni<nImages)
    {
        ...
        ros::spinOnce();
	    loop_rate.sleep();
        // SLAM.SaveTrajectoryTUM("/home/hero/catkin_ws/src/DS_SLAM2/Examples/CameraTrajectory.txt");
    }
    ...
    cout << "-------" << endl << endl;
    //cout << "vTimesTrack[nImages-1]: " << vTimesTrack[nImages-1]<< endl;
    cout << "median tracking time: " << vTimesTrack[nImages/2] << endl;
    cout << "mean tracking time: " << (totaltime-vTimesTrack[nImages-1])/(nImages-1) << endl;
    cout << "mean orb extract time =" << orbTotalTime/nImages <<  endl;
    cout << "mean moving detection time =" << movingTotalTime/nImages<<  endl;
    cout << "mean segmentation time =" << segmentationTime/(nImages-1)<<  endl;
