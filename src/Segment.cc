...
namespace ORB_SLAM2
{
Segment::Segment(const string &pascal_mean, const string &pascal_weights, const string &pascal_frac,const string &pascal_png, const string &pascal_aocx):mbFinishRequested(false),mSkipIndex(SKIP_NUMBER),mSegmentTime(0),imgIndex(0)
{

    mean_file = pascal_mean;
    weights_file = pascal_weights;
    frac_file=pascal_frac;
    LUT_file = pascal_png;
    aocx_file = pascal_aocx;
...
void Segment::Run()
{
    classifier=new Classifier(mean_file, weights_file,frac_file,aocx_file);
    ...
    double t_fpgainit= std::chrono::duration_cast<std::chrono::duration<double> >(tc2 - tc1).count();
    cout << "initialize fpga time  =" << t_fpgainit*1000 << "ms" << endl;
    while(1)
    {
